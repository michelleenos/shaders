// reference from threejs journey shader patterns lesson
...
void main() {
    ...
    // pattern 11 
    // float strength_x = step(0.8, mod(vUv.x * 10.0, 1.0));
    // float strength_y = step(0.8, mod(vUv.y * 10.0, 1.0));
    ...
    // pattern 14 
    // mine: 
    // vec2 fuv = fract(vUv * 10.0);
    // float strength = step(0.8, fuv.x) * step(0.3, fuv.y);
    // strength += step(0.8, fuv.y) * step(0.3, fuv.x);
    ...
    // mine:
    // vec2 fuv = fract(vUv * 10.0);
    // float strength = step(0.8, fuv.y) * step(0.3, fuv.x);
    // vec2 fuv2 = vec2(fract(fuv.x + 0.25), fract(fuv.y - 0.25));
    // strength += step(0.8, fuv2.x) * step(0.3, fuv2.y);
    ...
    // pattern 34
    // float strength = distance(vUv, vec2(0.5)) - 0.25;
    // strength = abs(strength);

    // pattern 35 
    // float c2 = distance(vUv, vec2(0.5)) - 0.25;
    // float strength = step(0.01, abs(c2));

    // pattern 36 
    // float c2 = distance(vUv, vec2(0.5)) - 0.25;
    // float strength = 1.0 - step(0.01, abs(c2));

    // pattern 37 
    // vec2 waveUv = vec2(vUv.x, vUv.y + sin(vUv.x * 30.0) * 0.1);
    // float c2 = distance(waveUv, vec2(0.5)) - 0.25;
    // float strength = 1.0 - step(0.01, abs(c2));
    ...
    // float strength = 1.0 - step(0.01, abs(distance(vUv, vec2(0.5)) - radius)); // from pattern 36
